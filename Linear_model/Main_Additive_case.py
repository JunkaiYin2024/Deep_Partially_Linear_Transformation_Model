        ...
        for i2 in range(len(c_rate)):
            for i3 in range(len(n_set)):
                ...
                for i in range(n_sim):
                    z, x, gx_true, time, delta = DataGenerator(n = n, r = r, c = c, sim = i, set = 'train', case = 'Additive')
                    ...
                    parameters = fit_model(r, z, x, t_splines, t_spline_derivatives, delta, n_iter = 100)
                    ...
                    x_coefs = parameters[z.shape[1] + n_splines_H: ]
                    gx = np.dot(x, x_coefs)
                    g_bar = np.mean(gx)
                    ...
                    z_test, x_test, gx_true_test, time_test, delta_test = DataGenerator(n = n_test, r = r, c = c, sim = i, set = 'test', case = 'Additive')
                    gx_test = np.dot(x_test, x_coefs)
