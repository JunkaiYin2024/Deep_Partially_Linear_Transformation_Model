...
def LogLikelihood(beta, gamma_tilde, spline_coefs, r, z, x_splines, t_splines, t_spline_derivatives, delta):
    ...
    phi_t = H_t + np.dot(z, beta) + np.dot(x_splines, spline_coefs)
...
def fit_model(r, z, x_splines, t_splines, t_spline_derivatives, delta, n_iter):
    z_dim = z.shape[1]
    x_splines_dim = x_splines.shape[1]
    n_splines_H = t_splines.shape[1]
    def loss_fn(parameters):
        beta = parameters[: z_dim]
        gamma_tilde = parameters[z_dim: z_dim + n_splines_H]
        spline_coefs = parameters[z_dim + n_splines_H: ]
        return LogLikelihood(beta, gamma_tilde, spline_coefs, r, z, x_splines, t_splines, t_spline_derivatives, delta)
    for i in range(n_iter):
        if i == 0:
            initial_values = np.zeros(z_dim + n_splines_H + x_splines_dim)
