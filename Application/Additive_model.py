...
def Additive_Estimation(r, z_train, z_test, x_train, x_test, time_train, time_test, delta_train, delta_test):
    ...
    node_list_g = np.linspace(0, 1, n_splines_g - 1)
    x_splines_train = np.array([[natural_spline(l, 3, node_list_g, x_train[j, k]) for k in range(x_dim) for l in range(n_splines_g)] for j in range(n)])
    x_splines_test = np.array([[natural_spline(l, 3, node_list_g, x_test[j, k]) for k in range(x_dim) for l in range(n_splines_g)] for j in range(n_test)])
                
    parameters = fit_model(r, z_train, x_splines_train, t_splines_train, t_spline_derivatives_train, delta_train, n_iter = 10)
    ...
    gx = np.dot(x_splines_train, g_coefs)
    ...
    gx_test = np.dot(x_splines_test, g_coefs)
